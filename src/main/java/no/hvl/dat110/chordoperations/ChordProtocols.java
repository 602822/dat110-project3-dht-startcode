...
public class ChordProtocols {
	...
	public void fixFingerTable() {
		...
		try {
			...
			List<NodeInterface> nodeFingerTable = chordnode.getFingerTable();

			// ensure to clear the current finger table
			nodeFingerTable.clear();
			...
			for (int i = 0; i < numberOfBits; i++) {
				...
				BigInteger nodeK = chordnode.getNodeID().add(BigInteger.valueOf(2).pow(i)).mod(adressSize);

				NodeInterface successorForNodeK = chordnode.findSuccessor(nodeK);

				// check that succnode is not null, then add it to the finger table
				if (successorForNodeK != null) {
					nodeFingerTable.add(successorForNodeK);
				}
			...
			//
		} catch (NoSuchAlgorithmException | NotBoundException e) {
