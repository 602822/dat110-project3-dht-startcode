...
public class FileManager {
	...
	public void createReplicaFiles() throws NoSuchAlgorithmException {
		...
		for(int i = 0; i < Util.numReplicas; i++) {
			BigInteger hashedFilenameAsBigInt = Hash.hashOf(filename + i);
			replicafiles[i] = hashedFilenameAsBigInt;
		}
	 ...
	 */
	public int distributeReplicastoPeers() throws RemoteException, NoSuchAlgorithmException, NotBoundException {
	 ...
	 * @throws RemoteException
	 */
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException, NoSuchAlgorithmException, NotBoundException {
		...
		for(int i = 0; i < numReplicas; i++) {
			BigInteger replicafile = replicafiles[i];
			NodeInterface successor = chordnode.findSuccessor(replicafile);
			Message filesMetadata = successor.getFilesMetadata(replicafile);
			activeNodesforFile.add(filesMetadata);
		}
